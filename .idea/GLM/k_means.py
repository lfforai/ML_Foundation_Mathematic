...
def k_means_core(u=tf.constant(0),y=tf.constant(0),w=tf.constant(0),means_group_num=5,row_size=4000,step=50):
...
#基于tensorflow的k_means
def k_means(filename="",parameter=[]):
    #计算参数
    g=tf.Graph()
    #计算参数GLM模型参数
    b_1=GLM_iteration.train_tweedie_model2b_1(filename="/home/mapd/dumps/output/GLM_base_date_90Days_one_hot.csv",arr_len_sum=27,loop_times=50)
    print("GLM模型参数：",b_1)
    ...
    with g.as_default():
         ...
         point=k_means_core(u,y,w,means_group_num=5,row_size=row_size)
         import pandas as pd
