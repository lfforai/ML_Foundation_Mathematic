...
def main(_):
    ...
    #
    w_gamma=tf.get_variable(name='tweedie_var', shape=[arr_len_sum], initializer=tf.random_normal_initializer(mean=0, stddev=1))
    ...
    output_y_p=tf.exp(tf.reduce_sum(tf.multiply(x_batch, w_gamma),axis=1))
    output_y=y_batch
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.Session() as sess:
            ...
            #     #lobal_step = ckpt.model_checkpoint_path.split('/')[-1].split('-')[-1]
            #      global_step='/temp/lf/model.ckpt-157218'
            # else:
            #     print('No checkpoint file found')
            saver.restore(sess, '/temp/lf2/model.ckpt-900')
            ...
            for i in  range(1):
                ...
                if tweedie_mark==1:
                    print(sess.run(output_y_p))
                    print(sess.run(output_y))
                    print("-------------------------------")
                    print(sess.run(w_gamma))
                    print("-----------------")
                    print(sess.run(output_y_p-output_y))
                # if poisson_mark==1:
